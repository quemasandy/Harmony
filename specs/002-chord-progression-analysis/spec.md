...
- Q: Does `Progression` accept raw strings and parse them, or does it accept pre-validated `Chord` objects? → A: `Progression` expects pre-instantiated valid `Chord` objects; parsing and rejecting invalid strings happens in an outer layer.
- Q: What should the `RomanNumeral.symbol` contain when a chord is identified as non-diatonic? → A: The analysis of each chord is a discriminated union (`{ diatonic: true, romanNumeral: RomanNumeral }` or `{ diatonic: false }`). The `RomanNumeral` VO only exists for diatonic functions.
- Q: Should SC-001 match raw music21 or normalized output? → A: SC-001 matches the *normalized* music21 output (using the existing python wrapper script to align with modern notation).
...
- How does the system handle chords that cannot be parsed? It rejects them in the outer parsing layer before passing valid `Chord` objects to the `Progression`.
...
- **FR-007**: The system MUST reject empty progressions with clear error messages. Parsing of string inputs and rejection of invalid chords happens in an outer layer before passing `Chord` objects to the `Progression`.
...
- **SC-001**: 100% of Roman numerals generated by the system match the *normalized* output of music21 (via the existing python wrapper script) for the same chord and key.
